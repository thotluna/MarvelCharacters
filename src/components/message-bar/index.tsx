...
export const MessageBar = component$(() => {
  ...
  useVisibleTask$(({ track, cleanup }) => {
    ...
    if (storageMessage.type === MessageType.SUCCESS)
      bg.value = "bg-emerald-500";
  ...
  return (
    <article
      class={`ticky bottom-4 right-0 left-0 h-10 mx-4 opacity-75 p-4 rounded-sm border-none flex items-center justify-between overflow-hidden ${bg.value} `}
    >
      <p class="flex-1">{storageMessage.message}</p>
      <button onClick$={() => handlerStorageMessage(MessageStoreDefaults)}>
        {!storageMessage.requiredTimeout && <Cancel width={24} height={24} />}
      </button>
    </article>
  );
});
